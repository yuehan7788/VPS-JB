...
show_script_info() {
    ...
    printf "${yellow}%-30s${none} = %s\n" "脚本安装路径${none} = /usr/local/bin/VPS-JB.sh"
    printf "${yellow}%-30s${none} = %s\n" "别名配置文件${none} = /etc/profile.d/vps-jb-bieming.sh"
    printf "${yellow}%-30s${none} = %s\n" "软链接路径${none} = /usr/local/bin/vps-jb"
    printf "${yellow}%-30s${none} = %s\n" "快捷命令${none} = y 或 vps-jb"
    printf "${yellow}%-30s${none} = %s\n" "无响应中断操作${none} = Ctrl+C"
    printf "${yellow}%-30s${none} = %s\n" "Xray(233boy急速)${none} = 命令 xray"
    printf "${yellow}%-30s${none} = %s\n" "八合一键脚本mack-a${none} = (歇斯底里) & 命令 vasma"
    printf "${yellow}%-30s${none} = %s\n" "kejilong工具${none} = 命令 k"
    echo -e "${cyan}========================================${none}"
    
    echo -e "\n"
    
    echo -e "${cyan}========================================${none}"
    echo -e "${green}        脚本信息 (方式四)${none}"
    echo -e "${cyan}========================================${none}"
